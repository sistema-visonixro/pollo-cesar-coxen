...
export default function AperturaView({ usuarioActual, caja }: AperturaViewProps) {
  ...
  const registrarApertura = async () => {
    ...
    if (!caja || caja === '' || caja === null || caja === undefined) {
      setError('No se puede registrar apertura: el valor de "caja" es nulo o vacío. Contacte al administrador.');
      setLoading(false);
      return;
    }
    const hoy = new Date();
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f5f5' }}>
      <button
        style={{ fontSize: 28, padding: '24px 48px', borderRadius: 16, background: '#1976d2', color: '#fff', fontWeight: 700, border: 'none', boxShadow: '0 2px 12px #1976d222', cursor: 'pointer' }}
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 8px 32px #1976d222', padding: 32, minWidth: 350 }}>
            <h2 style={{ color: '#1976d2', marginBottom: 18 }}>Fondo Fijo de Caja</h2>
            <input
              type="number"
              value={fondoFijo}
              onChange={e => setFondoFijo(e.target.value)}
              placeholder="Ingrese fondo fijo"
              style={{ padding: '12px', borderRadius: 8, border: '1px solid #ccc', fontSize: 18, marginBottom: 18, width: '100%' }}
            />
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
              <button
                onClick={registrarApertura}
                disabled={loading || !fondoFijo}
                style={{ background: '#1976d2', color: '#fff', borderRadius: 8, border: 'none', padding: '10px 32px', fontWeight: 700, fontSize: 18, cursor: 'pointer' }}
              >Registrar</button>
              <button
                onClick={() => setShowModal(false)}
                style={{ background: '#d32f2f', color: '#fff', borderRadius: 8, border: 'none', padding: '10px 32px', fontWeight: 700, fontSize: 18, cursor: 'pointer' }}
              >Cancelar</button>
            </div>
